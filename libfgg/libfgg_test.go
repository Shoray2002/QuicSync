...
import (
	...
	"quicsync/libfgg/pool"
	"quicsync/libfgg/transport/socket"
)

func TestLibFgg(t *testing.T) {
	// 100M
	sendPath, _, err := pool.HelpCreateTmpFile("quicsync_libfgg_send", 100*1024*1024)
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(sendPath)

	recvPath := path.Join(os.TempDir(), "quicsync_libfgg_recv")
